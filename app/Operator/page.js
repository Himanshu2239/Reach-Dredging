...
export default function page() {
  ...
  const dredgerReadings = {
    K7: ["M/E Reading", "L/C Reading", "D/G Reading"],
    K9: ["M/E Reading", "L/C Reading", "D/G Reading"],
    K14: ["M/E Reading", "L/C Reading", "A/E Reading", "D/G Reading"],
    K15: ["M/E Reading", "A/E Reading", "D/G Reading"],
  };

  const handleDredgerChange = (e) => {
    const selectedDredger = e.target.value;
    setDredger(selectedDredger);
    setSelectedReadings(dredgerReadings[selectedDredger] || []);

    // Reset values for new dredger selection
    setReadingValues({});
  };
  ...
  const handleSubmit = async () => {
    // console.log("readingValue", readingValues, "   ", selectedReadings);
    if (!date || !shift || !dredger) {
      toast.error("Date, shift, and dredger type are required.");
      return;
    }
    if (!forward || !swing || !depth) {
      toast.error("Please fill in all required work log fields.");
      return;
    }
    if (entries.some(entry => !entry.startTime || !entry.endTime)) {
    ...
    const requestBody = {
      ...
      workLog: {
        ...
        swing,
        depth,
        meReading: readingValues['M/E Reading'],
        aeReading: readingValues['A/E Reading'],
        dgReading: readingValues['D/G Reading'],
        lcReading: readingValues['L/C Reading'],
        entries,
      },
      oilReport: oilData,
    };
      ...
      <div className="min-h-screen bg-cover pt-[6rem] bg-center p-5" style={{ backgroundImage: `url(${images[0]})` }}>
        ...
        <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-3xl mx-auto">
          ...
          <div className="md:grid md:grid-cols-3 md:space-y-0 space-y-2 gap-4 mb-6">
            ...
            <div className="flex">
              <label className="text-lg font-medium mb-1 mr-2">Dredger: </label>
              <select value={dredger}
                onChange={(e) => {
                  setDredger(e.target.value)
                  handleDredgerChange(e)
          ...
          {/* <div className="grid grid-cols-3 gap-4 mt-4">
            <label>M/E Reading</label>
            <label>{dredger === 'K15' ? "A/E Reading" : "L/C Reading"}</label>
            <label>D/G Reading</label>
            <input type="number" value={meReading} onChange={(e) => setMeReading(e.target.value)} className="p-2 border rounded" />
            <input type="number" value={aeReading} onChange={(e) => setAeReading(e.target.value)} className="p-2 border rounded" />
            <input type="number" value={dgReading} onChange={(e) => setDgReading(e.target.value)} className="p-2 border rounded" />
          ...
          <div className={`grid gap-4 mt-4 ${dredger === "K14" ? "grid-cols-1 sm:grid-cols-2 md:grid-cols-4" : "grid-cols-1 sm:grid-cols-2 md:grid-cols-3"
            }`}>
            {selectedReadings.map((reading) => (
              <div key={reading} className="flex flex-col">
                <label className="text-sm font-medium">{reading}</label>
                <input
                  type="number"
                  value={readingValues[reading] || ""}
                  onChange={(e) => setReadingValues({ ...readingValues, [reading]: e.target.value })}
